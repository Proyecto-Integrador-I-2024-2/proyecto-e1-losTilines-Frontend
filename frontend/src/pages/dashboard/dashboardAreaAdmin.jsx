...
export function DashboardAreaAdmin() {
  ...
  console.log("user id and company", {
    worker: user.id,
  ...
  const {
    ...
    error: projectsError,
  } = useProjects({ company: user.company, area: user.area.id }); // Fetch projects by area

  let workersFilteredBusinessManager = [];

  if (!isLoadingWorkers && !isWorkersError) {
    console.log("Workers before filtering:", workers);
    console.log("User ID:", user.id);
  
    workersFilteredBusinessManager = workers.filter((item) => {
      const condition = item.role !== "Business Manager" && item.id !== user.id;
      return condition;
    });
        ...
        <section className="flex flex-col h-auto min-h-0 w-full mb-2 my-4 md:my-0 md:flex-row md:h-1/2  ">
          ...
          <section className="flex flex-col h-auto w-full md:space-x-6  md:flex-row  md:h-full">
                ...
                {!isLoadingWorkers ? (
                  ...
                  workersFilteredBusinessManager.length > 0 ? (
                    ...
                    <div className="flex flex-row justify-center items-center">
                      {isWorkersError ? (
                        <Typography>
                          There was an error loading the workers
                        </Typography>
                      ) : (
                        <Typography>There aren't elements to show.</Typography>
                      )}
            ...
            <ListCard title={"Finance"} hasAdd={false} hasSeeAll={() => {}}>
              <NumberInfo
                description={"Total investment in projects of the area"}
                number={`$${totalBudget}`}
              />
            </ListCard>
          </section>
