    ...
    def handle(self, *args, **options):
        ...
        for i, company in enumerate(companies):
            ...
            try:
                ...
                user_company2, _ = UserCompany.objects.get_or_create(
                    company=company,
                    user=admin,
                    defaults={
                        'area': area
                ...
                user_company3, _ = UserCompany.objects.get_or_create(
                    company=company,
                    user=manager,
                    defaults={
                        'area': area
                ...
                self.stdout.write(self.style.SUCCESS(f'Usuarios asociados a la compañía "{company.name}"'))
            except ValueError as e:
                self.stdout.write(self.style.ERROR(f'Error al crear UserCompany: {str(e)}'))
                continue
            user_companies.extend([user_company, user_company2, user_company3])
        ...
        for i, freelancer in enumerate(freelancers_instances):
            ...
            try:
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(f'Habilidad "{skill.name}" asignada a "{freelancer.user.email}"'))
                else:
                    self.stdout.write(self.style.WARNING(f'Habilidad "{skill.name}" ya asignada a "{freelancer.user.email}"'))
            except ValueError as e:
        ...
        for i, freelancer in enumerate(freelancers_instances):
            try:
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(f'Experiencia "{experience.occupation}" para "{freelancer.user.email}" creada'))
                else:
                    self.stdout.write(self.style.WARNING(f'Experiencia "{experience.occupation}" ya existe para "{freelancer.user.email}"'))
            except ValueError as e:
        ...
        project_statuses = []
        status_names = ['Pendiente', 'En progreso', 'Completado']
        for name in status_names:
            project_status, created = Status.objects.get_or_create(name=name)
            if created:
                self.stdout.write(self.style.SUCCESS(f'Estado "{name}" creado'))
            else:
                self.stdout.write(self.style.WARNING(f'Estado "{name}" ya existe'))
        ...
        # **Crear Milestones y Deliverables**
        milestones = []
        deliverables = []
        for i, project in enumerate(projects):
            freelancer = freelancers_instances[i % len(freelancers_instances)]
            num_milestones = 2  # Puedes ajustar el número de milestones por proyecto
            for m in range(1, num_milestones + 1):
                milestone_name = f'Milestone {m} - {project.name}'
                milestone_description = f'Descripción del milestone {m} para el proyecto {project.name}'
                due_date = timezone.now().date() + timezone.timedelta(days=15 * m)
                milestone_status = project_statuses[m % len(project_statuses)]
                try:
                    ...
                    if created:
                        self.stdout.write(self.style.SUCCESS(f'Milestone "{milestone.name}" creado para el proyecto "{project.name}"'))
                    else:
                        self.stdout.write(self.style.WARNING(f'Milestone "{milestone.name}" ya existe para el proyecto "{project.name}"'))
                    milestones.append(milestone)

                    # Crear Deliverables para cada Milestone
                    num_deliverables = 2  # Puedes ajustar el número de deliverables por milestone
                    for d in range(1, num_deliverables + 1):
                        deliverable_name = f'Deliverable {d} - {milestone.name}'
                        deliverable_description = f'Descripción del deliverable {d} para el milestone {milestone.name}'
                        ...
                        if created:
                            self.stdout.write(self.style.SUCCESS(f'Deliverable "{deliverable.name}" creado para el milestone "{milestone.name}"'))
                        else:
                            self.stdout.write(self.style.WARNING(f'Deliverable "{deliverable.name}" ya existe para el milestone "{milestone.name}"'))
                        deliverables.append(deliverable)
                except ValueError as e:
                    self.stdout.write(self.style.ERROR(f'Error al crear Milestone o Deliverable: {str(e)}'))
                    continue
